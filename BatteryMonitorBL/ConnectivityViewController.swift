...
extension ConnectivityViewController: UITableViewDataSource {
    ...
    /// ИСПРАВЛЕНИЕ #6 (02.10.2025): увеличен timeout до 10 сек, используется fallback для независимой загрузки
    /// ИСПРАВЛЕНИЕ #9 (03.10.2025): УБРАНА задержка 1.5 сек - пользователь не должен ждать!
    ///
    /// ПРОБЛЕМА: Задержка 1.5 сек прерывалась если пользователь быстро возвращался на Home
    ///           DispatchQueue.asyncAfter с [weak self] → self становился nil → загрузка не происходила
    /// РЕШЕНИЕ: Убираем задержку полностью, загружаем сразу, используем timeout 10 сек
    private func loadProtocolsAfterConnection() {
        ...
        AppLogger.shared.info(
            screen: AppLogger.Screen.connectivity,
            event: AppLogger.Event.dataUpdated,
            message: "[PROTOCOL_DEBUG] 🚀 Loading protocols immediately after connection",
            details: ["deviceName": deviceName]
        )

        // Загружаем Module ID (timeout 10 сек)
        ZetaraManager.shared.getModuleId()
            ...
            .subscribe(onSuccess: { idData in
                ...
                AppLogger.shared.info(
                    screen: AppLogger.Screen.connectivity,
                    event: AppLogger.Event.dataUpdated,
                    message: "[PROTOCOL_DEBUG] ✅ Module ID loaded: \(idData.readableId())",
                    details: ["deviceName": deviceName]
                )

                // Загружаем RS485 (timeout 10 сек)
                ZetaraManager.shared.getRS485()
                    ...
                    .subscribe(onSuccess: { rs485Data in
                        ...
                        AppLogger.shared.info(
                            screen: AppLogger.Screen.connectivity,
                            event: AppLogger.Event.dataUpdated,
                            message: "[PROTOCOL_DEBUG] ✅ RS485 loaded: \(rs485Data.readableProtocol())",
                            details: ["deviceName": deviceName]
                        )

                        // Загружаем CAN (timeout 10 сек)
                        ZetaraManager.shared.getCAN()
                            ...
                            .subscribe(onSuccess: { canData in
                                ...
                                AppLogger.shared.info(
                                    screen: AppLogger.Screen.connectivity,
                                    event: AppLogger.Event.dataUpdated,
                                    message: "[PROTOCOL_DEBUG] ✅ CAN loaded: \(canData.readableProtocol())",
                                    details: ["deviceName": deviceName]
                                )
                                ...
                                AppLogger.shared.error(
                                    ...
                                    message: "[PROTOCOL_DEBUG] ❌ CAN load failed: \(error.localizedDescription)",
                                    details: ["deviceName": deviceName]
                                )

                                // Даже если CAN упал, отправляем уведомление (Module ID и RS485 загружены)
                                NotificationCenter.default.post(
                                    name: HomeViewController.protocolsDidUpdateNotification,
                                    object: nil
                                )
                        ...
                        AppLogger.shared.error(
                            screen: AppLogger.Screen.connectivity,
                            event: AppLogger.Event.errorOccurred,
                            message: "[PROTOCOL_DEBUG] ❌ RS485 load failed: \(error.localizedDescription)",
                            details: ["deviceName": deviceName]
                        )

                        // Даже если RS485 упал, пытаемся загрузить CAN
                        ZetaraManager.shared.getCAN()
                            .timeout(.seconds(10), scheduler: MainScheduler.instance)
                            .subscribe(onSuccess: { canData in
                                ZetaraManager.shared.cachedCANData = canData
                ...
                AppLogger.shared.error(
                    screen: AppLogger.Screen.connectivity,
                    event: AppLogger.Event.errorOccurred,
                    message: "[PROTOCOL_DEBUG] ❌ Module ID load failed: \(error.localizedDescription)",
                    details: ["deviceName": deviceName]
                )

                // Даже если Module ID упал, пытаемся загрузить RS485 и CAN
                ZetaraManager.shared.getRS485()
                    .timeout(.seconds(10), scheduler: MainScheduler.instance)
                    .subscribe(onSuccess: { rs485Data in
                        ZetaraManager.shared.cachedRS485Data = rs485Data
                        ...
                        // Пытаемся загрузить хотя бы CAN
                        ZetaraManager.shared.getCAN()
                            .timeout(.seconds(10), scheduler: MainScheduler.instance)
                            .subscribe(onSuccess: { canData in
                                ZetaraManager.shared.cachedCANData = canData
                                ...
                                AppLogger.shared.error(
                                    screen: AppLogger.Screen.connectivity,
                                    event: AppLogger.Event.errorOccurred,
                                    message: "[PROTOCOL_DEBUG] ❌ All protocols failed to load",
                                    details: ["deviceName": deviceName]
                                )
                                NotificationCenter.default.post(
                                    name: HomeViewController.protocolsDidUpdateNotification,
